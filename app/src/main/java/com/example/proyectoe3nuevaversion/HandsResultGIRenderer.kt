...
class HandsResultGlRenderer : ResultGlRenderer<HandsResult?> {
    ...
    override fun renderResult(result: HandsResult?, projectionMatrix: FloatArray?) {
        ...
        for (i in 0 until numHands) {
            val landmarksList: NormalizedLandmarkList = result.multiHandLandmarks().get(i)
            val landmarks: List<NormalizedLandmark> = landmarksList.getLandmarkList()

            // Obtener landmarks del pulgar y del meñique
            val thumbTip: NormalizedLandmark = landmarks[4]
            val pinkyTip: NormalizedLandmark = landmarks[20]

            // Determinar la mano izquierda o derecha
            val handSide =
                if (thumbTip.getX() < pinkyTip.getX()) LEFT_HAND_CONNECTION_COLOR else RIGHT_HAND_CONNECTION_COLOR

            // Verificar si es la palma de la mano
            val wristLandmark: NormalizedLandmark = landmarks[0]
            val wristLandmarkZ: Float = wristLandmark.getZ()
            val isPalm: Boolean = wristLandmarkZ > landmarks[9].getZ()
            if (isPalm) {
                drawConnections(landmarks, handSide)
                for (landmark in landmarks) {
                    drawCircle(landmark.getX(), landmark.getY(), handSide)
                    drawHollowCircle(landmark.getX(), landmark.getY(), handSide)
                }
            }
        }
    }
